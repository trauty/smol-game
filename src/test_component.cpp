...
namespace smol_game
{
    void test_component_t::start()
    {
        ...
        asset_handle_t handle = asset_manager::load_sync<texture_asset_t>("assets/uiia_cat.png", texture_asset_args_t {
        texture_type_e::BASE_COLOR }); asset_ptr_t<texture_asset_t> image = asset_manager::get<texture_asset_t>(handle);
        ...
        asset_manager::load_async<texture_asset_t>(
            ...
            [](asset_handle_t async_handle) {
                ...
                SMOL_LOG_INFO("TEST_COMPONENT", "Breite: {}; Höhe: {};", async_image->get_width(),
        async_image->get_height());
            }
        );

        asset_handle_t shader_handle =
        asset_manager::load_sync<shader_asset_t>("assets/shaders/default.vert|assets/shaders/default.frag");
    ...
    void test_component_t::update(f64 delta_time)
    {
        ...
        vec3_t scale = {30.0f * std::abs(std::sin((f32)smol::time::get_time_in_seconds())),
                        30.0f * std::abs(std::sin((f32)smol::time::get_time_in_seconds())),
                        30.0f * std::abs(std::sin((f32)smol::time::get_time_in_seconds()))};
