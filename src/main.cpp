...
int main()
{
...

    {
        ...
        SMOL_LOG_INFO("SMOL", "Vertices: {}", model_handle->vertex_count);
        SMOL_LOG_INFO("SMOL", "Indices: {}", model_handle->index_count);

        smol::asset_t<smol::texture_asset_t> tex_handle = smol::asset_manager_t::load<smol::texture_asset_t>(
            "assets/textures/pastry.png", smol::texture_format_e::SRGB);

        smol::material_t cube_mat;
        cube_mat.init(*shader_handle);
        cube_mat.set_texture("albedo_tex", "albedo_sampler", *tex_handle);
