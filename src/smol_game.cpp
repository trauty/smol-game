...
extern "C"
{
    SMOL_API void smol_game_init(smol::world_t* world)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            mat->set_sampler("albedo_sampler", sampler_type_e::LINEAR_REPEAT);
        ...
        lit_mat->set_sampler("albedo_sampler", sampler_type_e::LINEAR_REPEAT);
        lit_mat->set_property<float>("smoothness", 32.0f);
        ...
        load_scene_base_draw(world);
    }

    SMOL_API void smol_game_update(smol::world_t* world)
    {
        if (smol::input::get_key_down(input::key_t::Q))
        {
            SMOL_LOG_INFO("GAME", "Switching to scene 1");
            load_scene_base_draw(world);
        }

        if (smol::input::get_key_down(input::key_t::W))
        {
            SMOL_LOG_INFO("GAME", "Switching to scene 2");
            load_scene_blinn_phong(world);
        }
    }
